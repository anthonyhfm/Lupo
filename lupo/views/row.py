...
class Row(View):
    def get_win32_render(self, origin, hinst):
        ...
        self._view_height = max(heights) if self.style.height is None else self._view_height
    ...
    def show_win32_view(self):
        ...
        calculated_width += self.style.gap * (len(self.children) - 1) if self.style.gap is not None else 0
